...
const emptyCadastro = (): CadastroFormState => ({
  orixa_cabeca_id: '',
  qualidade_cabeca_id: '',
  orixa_corpo_id: '',
  qualidade_corpo_id: '',
  orixa_caminho_id: '',
  qualidade_caminho_id: '',
  orixa_passagem_id: '',
...
export function useMemberForm(editId: UUID | null, onSaved: () => Promise<void> | void) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e instanceof Error ? e.message : 'Erro ao carregar orixás.');
      } finally {
    ...
    (async () => {
      try {
        ...
        setCadastro({
          ...
          orixa_caminho_id: cad?.orixa_caminho_id ?? '',
          qualidade_caminho_id: cad?.qualidade_caminho_id ?? '',
          ...
          orixa_caminho_reza: cad?.orixa_caminho_reza ?? '',
  ...
  const validate = useCallback((): string | null => {
    ...
    const pairs: Array<[unknown, unknown]> = [
      ...
      [cadastro.orixa_caminho_id, cadastro.qualidade_caminho_id],
      [cadastro.orixa_passagem_id, cadastro.qualidade_passagem_id],
    ];
    for (const [o, q] of pairs) {
      if (!str(o) && str(q)) return 'Selecione o orixá antes da qualidade.';
    }
    for (const row of orumale) {
      if (!str(row.orixa_id) && str(row.qualidade_id)) {
        return 'Em Orumalé, selecione o orixá antes da qualidade.';
      }
    }
    return null;
  ...
  const addOrumale = useCallback(() => {
    setOrumale((prev) => [
      ...prev,
      { key: newKey(), orixa_id: '', qualidade_id: '', digina: '', data_feitura: '' },
    ]);
  }, []);
  ...
  }, [validate, editId, nome, dataNascimento, contato, email, signo, obs, cadastro, orumale, exus, onSaved]);

  const setCadastroField = useCallback((field: keyof CadastroFormState, value: string) => {
    setCadastro((prev) => {
      const next = { ...prev, [field]: value };
      if (field === 'orixa_cabeca_id') next.qualidade_cabeca_id = '';
      if (field === 'orixa_corpo_id') next.qualidade_corpo_id = '';
      if (field === 'orixa_caminho_id') next.qualidade_caminho_id = '';
      if (field === 'orixa_passagem_id') next.qualidade_passagem_id = '';
      return next;
    });
  }, []);
